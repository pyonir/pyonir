...
class PyonirSecurity:
    ...
    def __init__(self, request: PyonirRequest):
        self._user = None
        self._request: PyonirRequest = request
        self._route_config: RouteConfig = None
    ...
    @property
    def responses(self):
        return self.request.json_responses
    ...
    def verify_request_access(self, request: PyonirRequest = None):
        ...
        if self.is_denied:
            req.route_response = self.request.redirect_response(self.redirect_to or '/')
    ...
    def get_user_profile(self, user_email: str = None) -> Optional[PyonirUser]:
        ...
        user_account = self.user_model.from_file(user_account_path, app_ctx=self.request.ctx_app.app_ctx) if os.path.exists(user_account_path) else None
        return user_account
